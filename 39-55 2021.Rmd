...
prescriptions <- dbGetQuery(con,
                          ...
                          WHERE
                            ...
                            --AND 2021 - c.birthyear BETWEEN 40 AND 60
...
analyze_group <- function(df, group_id, group_type) {
  ...
  # Count records in the original df for training period
...
print(results_all %>% 
        select(group_type, group, train_records, rmse, rrmse, t_stat, p_value, mean_train_resid, mean_test_resid))
...
# Analyza trendu
```{r}
analyze_group_trend_extrapolate <- function(df, group_id, group_type) {
  monthly_data <- df %>%
    mutate(month = floor_date(presc_date, "month")) %>%
    group_by(month) %>%
    summarise(prescriptions = sum(count), .groups = "drop") %>%
  ...
  # RMSE and RRMSE for post-2020 fit vs extrapolated pre-2020 forecast
  rmse_post <- sqrt(mean(post_df$residuals^2))
  mean_pre_trend <- mean(pre_df$trend)
  rrmse_post <- rmse_post / mean_pre_trend

  # Compare residuals (actual post-2020 trend minus extrapolated pre-2020 trend)
  ttest <- t.test(post_df$residuals)

  # Prepare plot data combining all
  trend_df <- trend_df %>%
    mutate(
...
results_vacc <- prescriptions_clean %>%
  ...
  group_by(vaccinated) %>%
  group_split() %>%
  map(~ analyze_group_trend_extrapolate(.x, group_id = unique(.x$vaccinated), group_type = "Vaccinated")) %>%
  compact() %>%
  bind_rows()
...
results_sex <- prescriptions_clean %>%
  ...
  group_by(sex) %>%
  group_split() %>%
  map(~ analyze_group_trend_extrapolate(.x, group_id = unique(.x$sex), group_type = "Sex")) %>%
  compact() %>%
  bind_rows()
...
print(
  results_all %>%
    ...
    arrange(p_value) %>%
    mutate(
      p_value_pre = signif(p_value_pre, 3),
      p_value = signif(p_value, 3),
      r_squared_pre = round(r_squared_pre, 3),
      slope_pre = round(slope_pre, 4),
      slope_post = round(slope_post, 4),
      rmse_post = round(rmse_post, 3),
      rrmse_post = round(rrmse_post, 3),
...
ggplot(plot_data_all, aes(x = month)) +
  geom_line(aes(y = trend), color = "black", size = 0.8) +               # actual trend
  geom_line(aes(y = pre_fit), color = "blue", size = 1.2, linetype = "dashed") +   # pre-2020 fit extrapolated
  geom_line(aes(y = post_fit), color = "red", size = 1.2, linetype = "dotted") +   # post-2020 fit
  geom_vline(xintercept = as.Date("2020-01-01"), linetype = "solid", color = "gray40") +
  facet_wrap(~ paste(group_type, group, sep = ": "), scales = "free_y") +
  labs(
    title = "Deseasonalized Prescription Trends with Linear Fits",
    subtitle = "Black = Actual STL trend; Blue dashed = Pre-2020 extrapolated; Red dotted = Post-2020 fit",
    x = "Month",
    y = "Deseasonalized Prescriptions"
  ) +
  theme_minimal() +
  theme(
...
# Residualy podle vekove kategorie
...
ggplot(plot_data_age, aes(x = plot_data_month)) +
  ...
  facet_wrap(~ paste(plot_data_group_type, plot_data_group, sep = ": "), scales = "free_y") +
  labs(
    title = "Actual vs Forecasted Prescriptions by Age Class",
    subtitle = "Blue = Actual, Red = Forecast (pre-2020 ETS)",
    x = "Date", y = "Prescriptions"
  ) +
  theme_minimal()
...
plot_longer <- pivot_longer(plot_data_age,values_to = "value", names_to = "name", cols = c("plot_data_prescriptions", "plot_data_predicted"))
...
results_age <- prescriptions_clean %>%
  ...
  group_by(age_class) %>%
  group_split() %>%
  map(~ analyze_group_trend_extrapolate(.x, group_id = unique(.x$age_class), group_type = "Age Class")) %>%
  compact() %>%
  bind_rows()
...
first_presc <- dbGetQuery(con,
                          ...
                          WHERE
                            i.ins_start_date < '2015-01-01' AND i.ins_end_date = '9999-12-31'
                            AND 2021 - c.birthyear BETWEEN 20 AND 50
...
did_model <- lm(rate ~ vaccinated * post_covid, data = monthly_rates)
summary(did_model)
```
...
multi_presc <- dbGetQuery(con,
                          ...
                          FROM
                          ozp_prescriptions p
                          INNER JOIN ozp_clients c ON p.client_id = c.client_id AND 2021 - c.birthyear BETWEEN 20 AND 50
                          ...
                          INNER JOIN cpzp_clients c ON p.client_id = c.client_id AND 2021 - c.birthyear BETWEEN 20 AND 50
...
did_model <- lm(multi_rate ~ vaccinated * post_covid, data = did_data)
summary(did_model)
```
...
combined_vaccinated_prescribed <- dbGetQuery(con,
                                    ...
                                    AND v.date BETWEEN DATE '2021-05-05' - INTERVAL '7 days'
                                                       AND DATE '2021-05-05' + INTERVAL '7 days'
                                    AND 2021 - c.birthyear BETWEEN 20 AND 50
...
combined_unvaccinated_prescribed <- dbGetQuery(con,
                                    ...
                                    SELECT
                                    p.client_id,
                                    DATE '2021-05-05' AS vaccination_date,
                                    p.date AS event_date,
                                    'CPZP' as ins_company
                                    ...
                                    LEFT JOIN cpzp_ins_start_end s ON c.client_id = s.client_id
                                    WHERE 1 = 1
                                    AND s.ins_start_date < DATE '2021-05-05' - INTERVAL '1 year'
                                    AND s.ins_end_date > DATE '2021-05-05' + INTERVAL '1 year'
                                    AND 2021 - c.birthyear BETWEEN 20 AND 50
                                    AND p.date BETWEEN DATE '2021-05-05' - INTERVAL '1 year' AND DATE '2021-05-05' + INTERVAL '1 year'
                                    AND p.presc_order = 1
                                    AND c.n_vacc = 0
