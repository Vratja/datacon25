...
vaccinations <- dbGetQuery(con,
  ...
  filter(ins_start_date < date - years(1), ins_end_date > date + years(1), cohort == "C") %>%
  transmute(
    person_id = client_id,
...
corticoids <- dbGetQuery(con,
  ...
  filter(ins_start_date < decisive_date - years(1), ins_end_date > decisive_date + years(1), cohort == "C") %>%
  transmute(person_id = client_id, event_date = date)

sccs_data <- vaccinations %>%
  left_join(corticoids, by = "person_id")

sccs_data <- sccs_data %>%
  mutate(across(c(event_date, exposure_start_date, exposure_end_date, observation_start_date, observation_end_date),
                as.Date))

# Calculate day offsets from observation start
sccs_data <- sccs_data %>%
  mutate(
    ...
    observation_start_day = 0,  # by definition
    observation_end_day = as.integer(observation_end_date - observation_start_date)
  )

events_df <- sccs_data %>%
  select(indiv = person_id, eventday = event_day) %>%
  distinct()

exposures_df <- sccs_data %>%
  select(indiv = person_id, start = exposure_start_day, end = exposure_end_day) %>%
  distinct() %>%
  mutate(exposure = "vaccine")

# Observation period
obs_df <- sccs_data %>%
  select(indiv = person_id, start = observation_start_day, end = observation_end_day) %>%
  distinct()

# Create a data frame with all needed columns for standardsccs
sccs_input_df <- obs_df %>%
  select(indiv, start, end) %>%
  left_join(
    events_df %>% group_by(indiv) %>% summarise(eventday = list(eventday)),
    by = "indiv"
  ) %>%
  left_join(
    exposures_df %>%
...
# Now call standardsccs on this data frame, specifying the columns:
sccs_data_formatted <- standardsccs(
  data = sccs_input_df,
  indiv = "indiv",
  astart = "start",
  aend = "end",
  aevent = "eventday",
  aedrug = c("start_exp", "end_exp", "exposure")
)

# Run model: risk during exposure vs baseline
fit <- sccs(event ~ exposure, data = sccs_input)
