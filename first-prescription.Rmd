...
n <- cl100$client_id %>% unique() %>% length()
...
Pouze klienti se dvěma dávkami
...
result <- standardsccs(formula = event ~ vacc_day,indiv = id,astart = astart,aend = aend,
  aevent = presc_day,adrug = vacc_day,aedrug = vacc_day + 365,
  expogrp = list(c(-100, 0, 20, 30, 120, 250, 300, 365)), data = cl
)
result
```
...
```{r}
x <- cl %>% 
  transmute(dif = vacc_day - presc_day, age, sex, id) %>% 
  filter(abs(dif) < 120) %>% 
  mutate(difGr = cut(dif, c(-Inf, 0, 20, 60, Inf)), difAge = cut(age, c(0, 30, 60, Inf))) %>% 
  filter(!is.na(difGr))

tab <- table(difGr = x$difGr,age = x$difAge)
...
```{r}
presc %>%
  left_join(cl11, by = "client_id") %>% 
  mutate(age_gr = ntile(age, 5)) %>% 

ggplot(aes(x = month_start, y = count, colour = age_gr, fill = age_gr)) +
  geom_line() +
  geom_point() +
